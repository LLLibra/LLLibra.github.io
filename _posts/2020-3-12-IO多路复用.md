...
> #### ET模式(边缘触发)
>
就绪的事件只能处理一次，若没有处理完会在下次的其它事件就绪时再进行处理。而若以后再也没有就绪的事件，那么剩余的那部分数据也会随之而丢失。(ET模式只支持非阻塞的读写：为了保证数据的完整性。) **快但是可能会漏**
>  
...
创建一棵红黑树，树的节点就是上面的数据结构epoll_event
...
>
![](imgs/20200331-113043.png)

* *events就是就绪队列 一个结构体数组
